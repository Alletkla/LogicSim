import * as SRD from '@projectstorm/react-diagrams';
import { BoolNodeFactory } from './bool/boolNode/BoolNodeFactory';
import { BoolSourceNodeFactory } from './bool/boolSourceNode/BoolSourceNodeFactory';
import { BoolLinkFactory } from './bool/boolLink/BoolLinkFactory';
import { BoolPortModelFactory } from './bool/boolPort/BoolPortModelFactory';
import { BoolTargetNodeFactory } from './bool/boolTargetNode/BoolTargetNodeFactory';
import { WrapperNodeFactory } from './wrapperNode/WrapperNodeFactory';

export class Application {
	protected activeModel: SRD.DiagramModel;
	protected diagramEngine: SRD.DiagramEngine;

	constructor() {
		this.diagramEngine = SRD.default();
		this.diagramEngine.getNodeFactories().registerFactory(new BoolNodeFactory())
		this.diagramEngine.getNodeFactories().registerFactory(new BoolSourceNodeFactory())
		this.diagramEngine.getNodeFactories().registerFactory(new BoolTargetNodeFactory())
		this.diagramEngine.getNodeFactories().registerFactory(new WrapperNodeFactory())
		this.diagramEngine.getLinkFactories().registerFactory(new BoolLinkFactory())
		this.diagramEngine.getPortFactories().registerFactory(new BoolPortModelFactory())

		this.newModel();
	}

	public newModel() {
		this.activeModel = new SRD.DiagramModel();
		this.diagramEngine.setModel(this.activeModel);

		// //3-A) create a default node
		// var node1 = new SRD.DefaultNodeModel('Node 1', 'rgb(0,192,255)');
		// let port = node1.addOutPort('Out');
		// node1.setPosition(100, 100);

		// //3-B) create another default node
		// var node2 = new SRD.DefaultNodeModel('Node 2', 'rgb(192,255,0)');
		// let port2 = node2.addInPort('In');
		// node2.setPosition(400, 100);

		// // link the ports
		// let link1 = port.link(port2);

		// this.activeModel.addAll(node1, node2, link1);
	}

	public getActiveDiagram(): SRD.DiagramModel {
		return this.activeModel;
	}

	public getDiagramEngine(): SRD.DiagramEngine {
		return this.diagramEngine;
	}

	public setModel(model:SRD.DiagramModel){
		if (!model){
			throw new Error("Model must not be null")
		}
		this.activeModel = model
	}
}